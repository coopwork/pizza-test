export const fakePresents = [
  {
    "title": "Лучшее тесто",
    "description": "Мы создаем тесто только из отборной итальянской муки наивысшего качества",
    "icon": "./assets/images/icons/wheat.png"
  },
  {
    "title": "Лучшие повара",
    "description": "Пиццы готовят самые профессиональные итальянские повара",
    "icon": "./assets/images/icons/cook.png"
  },
  {
    "title": "Гарантия качества",
    "description": "Наша пиццерия получила множество наград и признаний по всему миру",
    "icon": "./assets/images/icons/quality.png"
  },
  {
    "title": "Отборные рецепты",
    "description": "Мы используем рецепты от мировых лидеров в изготовлении пиццы",
    "icon": "./assets/images/icons/dish.png"
  }
];

export const fakeProducts = [
  {
    "title": "Мясная Делюкс",
    "description": "Пепперони, лук, бекон, томатная паста, колбаски, перец, грибы, соус чили, ананасы",
    "image": "./assets/images/products/meat-delux.png"
  },
  {
    "title": "Морская Премиум",
    "description": "Перец, сыр, креветки, кальмары, мидии, лосось",
    "image": "./assets/images/products/sea-premium.png"
  },
  {
    "title": "Бекон и Сосиски",
    "description": "Бекон, сыр, сосиски, ананас, томатная паста",
    "image": "./assets/images/products/bekon-sausages.png"
  },
  {
    "title": "Куринная Делюкс",
    "description": "Курица, ананас, сыр Пепперони, соус для пиццы, томатная паста",
    "image": "./assets/images/products/chiken-delux.png"
  },
  {
    "title": "Барбейю Премиум",
    "description": "Свинина BBQ, соус Барбекю, сыр, курица, соус для пиццы, соус чили",
    "image": "./assets/images/products/bbq-premium.png"
  },
  {
    "title": "Пепперони Дабл",
    "description": "Пепперони, сыр, колбаса 2 видов: обжаренная и вареная",
    "image": "./assets/images/products/pepperoni-double.png"
  },
  {
    "title": "Куриное трио",
    "description": "Жареная курица, Тушеная курица, Куриные наггетсы, перец, сыр, грибы, соус для пиццы",
    "image": "./assets/images/products/chiken-trio.png"
  },
  {
    "title": "Сырная",
    "description": "Сыр Джюгас, сыр с плесенью, Сыр Моцарелла, Сыр секретный",
    "image": "./assets/images/products/cheese.png"
  }
]